...
# 데이터 경로 설정 (CSV 파일)
DATA_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", csv_file_path)
...
def main():
    ...
    # 가중치 추출: 행 1 (인덱스 1), 열 N, O, P, Q (인덱스 13~16)
    mbtiWeight = float(df.iloc[1, 13])      # 예: 0.2
    ageWeight = float(df.iloc[1, 14])       # 예: 0.2
    contactFrequencyWeight = float(df.iloc[1, 15])   # 예: 0.4
    hobbyWeight = float(df.iloc[1, 16])     # 예: 0.2
